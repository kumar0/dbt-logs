...
def render(classified: pd.DataFrame):
    ...
    if running.empty:
        st.info("No models currently in progress. All models have completed. "
                "To monitor a new run, enable auto-refresh before starting the Step Function.")
        return

    st.markdown(f"### 🔄 {len(running)} Model(s) Currently Running")
    for entity_name, entity_group in running.groupby("entity"):
        with st.expander(f"🏷️ {entity_name} — {len(entity_group)} model(s)", expanded=False):
            for _, row in entity_group.iterrows():
                ...
                c1.markdown(f"**Model:** `{row['model_name']}`")
