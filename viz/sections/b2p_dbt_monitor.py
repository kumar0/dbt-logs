"""dbt Monitor sub-tab for the Base to Prepared section.

Extracted from the original base_to_prepared.py to support sub-tab navigation.
Receives shared date/time controls as parameters.
"""
...
def render_b2p_dbt_monitor(start_date, start_time, end_date, end_time, auto_refresh) -> None:
    """Render the existing dbt monitoring dashboard as a sub-tab."""
    ...
    # Fetch button
    if st.button("🔍 Fetch Data", use_container_width=True, type="primary", key="b2p_dbt_fetch_btn"):
    ...
    # --- Fetch data only when explicitly requested (button or auto-refresh) ---
    ...
    if _should_fetch:
        ...
        if refresh_interval_s > 0 and fetch_mode == "delta":
            ...
            date_to = st.session_state.b2p_effective_to_date
        else:
