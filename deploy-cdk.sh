...
echo ""
echo "To trigger the full pipeline:"
echo "  aws stepfunctions start-execution \\"
echo "    --state-machine-arn arn:aws:states:${AWS_REGION}:${ACCOUNT_ID}:stateMachine:etl-dbt-pipeline \\"
echo "    --input '{\"dbtCommand\":\"build\",\"numWorkers\":\"3\",\"workerType\":\"G.1X\",\"entityName\":\"all\",\"runDate\":\"$(date +%d%m%Y)\"}' \\"
echo "    --profile ${AWS_PROFILE}"
echo ""
echo "To trigger a single entity (e.g. customers):"
echo "  aws stepfunctions start-execution \\"
echo "    --state-machine-arn arn:aws:states:${AWS_REGION}:${ACCOUNT_ID}:stateMachine:etl-dbt-pipeline \\"
echo "    --input '{\"dbtCommand\":\"build --select tag:customers\",\"numWorkers\":\"3\",\"workerType\":\"G.1X\",\"entityName\":\"customers\",\"runDate\":\"$(date +%d%m%Y)\"}' \\"
echo "    --profile ${AWS_PROFILE}"
